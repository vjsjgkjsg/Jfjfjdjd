...
══════════════════════════════════════════ */
const Records = {
  key: 'pedagog_records_v3',
  load() { try{ return JSON.parse(localStorage.getItem(this.key))||{}; }catch(e){ return {}; } },
  save(data) { try{ localStorage.setItem(this.key,JSON.stringify(data)); }catch(e){} },
  get(game) { return this.load()[game]||null; },
  set(game, val) {
    const d=this.load(); d[game]=val; this.save(d);
  },
  update(game, val, isLower=true) {
    const cur=this.get(game);
    if(cur===null || (isLower ? val<cur : val>cur)) {
      this.set(game,val); return true;
    }
    return false;
  }
};
...
const Achievements = {
  ...
  unlock(id) {
    ...
    const list=this.load(); list.push(id);
...
function puzzleInit() {
  ...
  $('puzzle-record').textContent = rec ? '🏆 Рекорд: '+rec+' ход' : '—';
...
function puzzleClick(idx) {
  ...
  if(tiles.every((v,i)=>v===i)){
    ...
    const isNew=Records.update('puzzle_'+grid, PS.moves);
    ...
    setTimeout(()=>{
      ...
      Audio.bigWin();
      Achievements.unlock('first_puzzle');
      if(grid===5) Achievements.unlock('hard_puzzle');
      if(grid===3&&PS.time<30) Achievements.unlock('speed_puzzle');
    },350);
  }
}
    ...
    if(MS.pairs[a.idx].e===MS.pairs[b.idx].e){
      ...
      setTimeout(()=>{
        ...
        if(MS.matched===total){
          ...
          const isNew=Records.update('memory_'+MS.set,MS.moves);
          ...
          setTimeout(()=>{
            ...
            Audio.bigWin();
            Achievements.unlock('first_memory');
            if(MS.moves<=total) Achievements.unlock('perfect_memory');
          },500);
        }
      },500);
    } else {
...
function quizAnswer(idx) {
  ...
  if(idx===q.c){
    ...
    if(QS.streak>=5) Achievements.unlock('quiz_5');
  } else {
    QS.streak=0;
...
function quizFinish() {
  ...
  const isNew=Records.update('quiz',s,false);
  ...
  if(s===10) Achievements.unlock('quiz_master');
  Audio.bigWin();
}
...
$('color-save').addEventListener('click',()=>{
  ...
  Audio.bigWin(); Achievements.unlock('first_color'); toast('🎉 Сохранено в загрузки!','3000','success');
});
...
function sortDrop(bucketId) {
  ...
  if($('sort-pool').children.length===0){
    const isNew=Records.update('sort_'+SS.mode,SS.errors);
    setTimeout(()=>{
      ...
      Audio.bigWin();
      if(SS.errors===0) Achievements.unlock('sort_perfect');
    },500);
  }
}
...
window.printCard=function(type){
  const labels={loto:'Лото «Животные Казахстана»',bajga:'Байга — степная скачка',map:'Пазл «Карта города»',svetofor:'Светофор (цветные круги)',bazar:'Деньги и ценники для базара',asyk:'Правила Асыка',ornament:'Раскраски с орнаментом',hot_cold:'Карточки достопримечательностей'};
  Audio.click();
  toast('🖨 «'+(labels[type]||type)+'» — добавьте PDF-файл на сервер!','4000','info');
};
...
function buildRecordsModal() {
  ...
  const unlocked=Achievements.load();
